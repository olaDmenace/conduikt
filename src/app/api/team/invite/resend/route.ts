    ...
    const { data: emailData, error: emailError } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: system-ui, sans-serif; max-width: 480px; margin: 0 auto;">
          <h2 style="color: #D9663A;">You're invited to Conduikt</h2>
          ...
          <a href="${process.env.NEXT_PUBLIC_APP_URL || "https://conduikt.com"}/signup?invite=${invite.id}&email=${encodeURIComponent(invite.invite_email)}"
             style="display: inline-block; padding: 12px 24px; background: #D9663A; color: #FFFFFF; border-radius: 8px; text-decoration: none; font-weight: 600; margin-top: 16px;">
            Accept Invite
          </a>
